...
export function BulkAddModal (props: {
  ...
  const onAddTerms = async () => {
    if (!topic.id) return
    await bulkAddItems({
      words: searchResults.filter(s => !!s.results[0]).map(s => ({ word: s.word, jishoData: s.results[0] })),
      topicId: topic.id
    })
    onAddSuccess()
  }

  const wordsFound = useMemo(() => searchResults.map(s => s.results.length > 0).length, [searchResults])
      ...
      <ModalContent>
          ...
          <ModalBody>
            <Stack>
                ...
                <Button isDisabled={isLoading} onClick={() => { onSearchTerms() }}>Search Terms</Button>
                {
                    searchResults.length > 0 && (
                        <Stack>
                            <Text>Search results found for {wordsFound} word(s) ({searchResults.length - wordsFound} missing)</Text>
                            <Button colorScheme="green" isDisabled={isLoading} onClick={() => { onAddTerms() }}>Add Terms</Button>
                        </Stack>
                    )
                }
            </Stack>
          ...
          <ModalFooter>
            <HStack>
              <Button isDisabled={isLoading} onClick={() => { onClose() }}>Close</Button>
            </HStack>
          </ModalFooter>
      </ModalContent>
    </Modal>
